...
excerpt: 'Yesterday I had a nightmare about a problem I bumped into while coding my new project.
...
coverImage: /assets/blog/you-dont-really-need-this-package/banner.jpeg
date: "2018-10-08T05:35:07.322Z"
author:
  name: Matan Borenkraout
  picture: /assets/blog/authors/matan.jpeg
ogImage:
  url: /assets/blog/you-dont-really-need-this-package/banner.jpeg
---
...
Lodash functionality can be very useful. But we got used to it and sometimes we use it when there’s actually no need to. ES6 contains a lot of functions that really make Lodash redundant.
...
[Moment.js](http://momentjs.com/) is a very powerful tool, but while it doesn’t include tree-shaking, you’re stuck with the whole library. Sometimes, all you need to do is to parse a date or format, and for that, you don’t really need a new package.
...
If you’re not ready to ditch the date library or you need some functionality that can’t be relied in the Date object, give [date-fns](https://date-fns.org/) a try as it includes tre-shaking and includes all functionality.

### Axios Vs Fetch(ES6)

There are many packages that came to solve our problem with sending HTTP request to the server when [fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch) wasn’t there and we only had [XMLHttpRequest](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest). They did their job but I believe it’s time to start implementing the new standard.  
Of course, this has some downsides as the API is still half-baked and while implementing it you will need to compromise on some stuff. Here’s an example of a request:
...
People usually claim the main problem with fetch is that you can’t cancel an ongoing request. But this problem already has a fix that was already [merged](https://github.com/whatwg/fetch/pull/523) to the fetch specification and is already in the [DOM standards](https://dom.spec.whatwg.org/#aborting-ongoing-activities), though some browsers still don’t support it. But it’s yet to come.  
